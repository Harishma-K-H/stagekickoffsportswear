...
const SignIn: React.FC = () => {
  ...
  const handleSubmit = async (values: {
    ...
    try {
      ...
      const { access, name, role, refresh, branch_id, email } = data;
      dispatch(setToken({ access, refresh }));
      dispatch(setUserName({ name }));
      dispatch(setUserRole(role.toUpperCase()));
      dispatch(setBranchDetails({ ...branch_id, email }));
  ...
  return (
    <>
      <Helmet>
        <title>KICKOFF SPORTS WEAR - Sign In </title>
      </Helmet>
      <div className="grid items-center min-h-screen grid-cols-1">
        <div className="w-full px-3 mx-auto md:px-0 md:w-auto">
          ...
          />
          <h3 className="mb-5 font-bold text-center 2xsm:text-2xl md:text-4xl xl:text-4xl">
